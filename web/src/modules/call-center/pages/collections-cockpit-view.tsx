...
export interface CollectionsCockpitViewProps {
  /** Fetch queue, summary, and workflow snapshot (GM/accountant hub passes true without `collections.view`). */
  dataEnabled: boolean;
  /** Hide workflow mutations, quick-add, debt conversion, and shortcut-rail actions. */
  isReadOnly?: boolean;
}

export function CollectionsCockpitView({
  dataEnabled,
  isReadOnly = false,
}: CollectionsCockpitViewProps): React.ReactElement {
  ...
  const workflow = useCollectionsWorkflow({ branchId, enabled: dataEnabled });
  ...
  }, [refreshQueue, refreshSummary, dataEnabled]);
  ...
  }, [focusedRow, queueRows, refreshQueue, workflow, isReadOnly]);
  ...
  return (
    ...
    >
      ...
      <WorkflowLanes
        ...
        readOnly={isReadOnly}
        onQuickAdd={
          isReadOnly ? undefined : (kind) => {
              if (focusedRow) setModalKind(kind);
            }
        }
        onClaim={isReadOnly ? undefined : handleClaim}
        onRelease={isReadOnly ? undefined : handleRelease}
        onComplete={isReadOnly ? undefined : handleComplete}
        onBreak={isReadOnly ? undefined : handleBreak}
        onCancel={isReadOnly ? undefined : handleCancel}
      />
      ...
      >
        <div className="min-w-0 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          ...
          <SmartDebtTable
            ...
            locale={locale}
            showConvertColumn={!isReadOnly && canManageSubscribers && canAct}
          ...
          {focusedRow ? (
            <div className="mt-2 space-y-1.5 text-xs">
              ...
              {!isReadOnly ? (
                ...
                <p className="rounded border border-dashed border-slate-200 px-2 py-1.5 text-[0.7rem] text-slate-500 dark:border-slate-600">
                  {isAr ? 'عرض فقط — الإجراءات غير متاحة لدورك.' : 'Read-only — actions are not available for your role.'}
                </p>
              )}
            </div>
          ) : (
            <p className="mt-2 text-xs text-slate-500">
              {isAr ? 'اختر صفًا من الطابور لمشاهدة التفاصيل.' : 'Pick a row to inspect queue details.'}
            </p>
          )}
      ...
      {!isReadOnly && modalKind && focusedRow ? (
        <WorkflowQuickAddModal
          open
