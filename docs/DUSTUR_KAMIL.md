...
> ### 📍 حالة الدستور — آخر تحديث: **2026-04-23**
>
> **الفرع النشط:** `main`
> **آخر Commit:** `d3a37ae` — V19.20 — CC mobile redesign, manager observability, payroll loan deductions (+ تصفير فواتير الإنتاج، V19.21) (+ مطابقة درج الموبايل للمسؤل، V19.22) (+ إدارة التسلسل تشمل مدراء الفروع، V19.23) (+ **تسلسل الفاتورة لكل مُشغّل** `prefix-n`، V19.24)
>
...
>
> (#1 تسديد جزئي، #2 رصيد مرحّل، #3 بحث بالهاتف، #4 محصل يومي، #5 فرع/سائق في الإشعارات،
> #6 إخفاء الإيجارات/السلف، #7 مديونية على الفاتورة، #8 تقارير 360°، #9 تحويل دين→اشتراك،
> #10 كشف حساب + حالة القطع، #11 ربط العميل بالاشتراكات، #12 كل اشتراك منفصل)
>
> #### ⏳ مهام عمليات غير مُبرمجة (تحتاج قرار أو بيئة حيّة)
>
> 1. تشغيل Canary فعلي (`scripts/canary-smoke.ps1` + `tests/canary-results.md`).
> 2. إعادة قفل ساعات الدخول: إلغاء `AUTH_BYPASS_WORKING_HOURS=true` قبل الإنتاج.
> 3. ضبط Sentry DSN الإنتاجي في `.env`.
> 4. جدولة `scripts/db-backup.ps1` في Task Scheduler على السيرفر.
> 5. Backfill سجلات `CustomerSubscription` للعملاء القدامى (Option 2-A / 3-A).
> 6. ربط جهاز الحضور البيومتري الحقيقي بالـ API stub.
>
> #### 📌 قواعد لا تُكسر عند استكمال العمل
>
> - **ملف `الدستور.ini`** في الجذر و**هذا الملف** (`DUSTUR_KAMIL.md`) لا يُحذفان ولا يُعاد ترتيبهما.
> - **صفحة المالك (Owner)** مجمّدة — المالك يُحدّث واجهتها يدويًا.
> - واجهة الكول سنتر تعمل من `/subscribers` و `/customers` و `/collections` — أي ميزة جديدة للكول سنتر **ترتبط بإحدى هذه الصفحات الثلاث** ولا تُضاف تحت `/subscriptions`.
> - حساب التفعيل + تسوية الدين: مصدر الحقيقة الوحيد هو `CustomerLedgerService.activateSubscriptionPlan`. أي معاينة جديدة لازم تُطابقه بالخانة.
...
>
> #### 🚀 V19.20 — نشر كامل على الإنتاج (Railway) — 2026-04-23
>
> **كوميت:** `d3a37ae` (main). 76 ملفاً (64 تعديل + 13 إضافة + 1 migration + إزالة artifacts قديمة من `dist/`).
>
> **الحزم المُشحنة داخل هذا الكوميت:**
>
> 1. **تصميم الموبايل الجديد (V19.15):** درج جانبي + هامبرغر عائم + إزالة الشريط السفلي + إزالة "أدوات الواتساب" من كل السايدبارات — شُحنت من فرع `main` لأول مرة هنا.
> 2. **مراقبة مدير الفرع:** صفحة "مستنداتي" (عهد كاش مُدقّقة + سندات مصروف مُعتمدة مع طباعة A4)، صفحة "مراقبة السائقين" (كروت ملوّنة + Map API فُتح للمدير والكول سنتر بنطاق الفرع)، "سجل الطلبات" أُعيدت تسميته إلى "الفواتير" مع فلاتر كاملة (سائق/حالة/كاش/دفع/تاريخ/بحث) مقيّد بفرع المدير.
> 3. **حارس الفواتير السريعة (Quick-create fraud mitigation):** حقل `posPaymentMethod` صار إلزامي في `CreateOrderQuickDto` — انسدّت ثغرة "طريقة الدفع غير محددة". Cron يومي (`StaleQuickOrdersCron`) + بانر أحمر على لوحة المحاسب يرصد أي فاتورة سريعة بقيت unpaid > 24 ساعة.
> 4. **روابط الدفع (UPayments):** `paymentLinkExpiryInMinutes = 1440` (24 ساعة). شارات حالة كشف المتابعة الميداني تبسّطت إلى حالتين: **قيد الانتظار** (أزرق — رابط حي < 24h) مقابل **غير مدفوعة** (أحمر — كل ما عدا). أُلغيت "بانتظار الاعتماد".
> 5. **V19.20 — خصم السلف من الرواتب:** قسط السلفة يُسجَّل كبند منفصل `Payroll.loanDeduction` ويُحفَظ `EmployeeLoan.lastDeductionYearMonth` عشان إعادة تشغيل مسير نفس الشهر ما يخصم القسط مرتين. Migration `20260423220000_payroll_loan_deduction` يضيف العمودين بأمان (default 0 / nullable — zero-downtime).
> 6. **توحيد مسار البناء:** `nest build` صار يُخرج إلى `dist/main.js` (بدلاً من `dist/src/main.js`). `tsconfig.build.json` أضاف `rootDir=./src`. `package.json` و `Dockerfile` اتماشوا مع المسار الجديد. ملفات `dist/` القديمة اتحذفت من git + `tsconfig.build.tsbuildinfo` انضاف لـ`.gitignore`.
> 7. **وثائق:** دستور السائق (`docs/DUSTUR_SAIQ_V1.md`) اتضاف كدفتر تشغيل مخصّص لرتبة السائق.
>
> Railway يلتقط `main` تلقائياً، يُعيد بناء الـimage، و `CMD` يُشغّل `npx prisma migrate deploy && node dist/main.js` فتسري الـmigration قبل بدء التطبيق.
>
> #### 🧹 V19.21 — تصفير سجلات الإنتاج قبل الانطلاق التجاري — 2026-04-23
>
> **السياق:** قبل التشغيل التجاري الفعلي، صُفِّرت كل الفواتير التجريبية والسجلات المالية المرتبطة بها من قاعدة Railway، مع إرجاع عدّاد الفواتير إلى الصفر.
>
> **الأداة:** `scripts/reset-invoices.ts` — سكريبت TypeScript يستخدم Prisma + `@prisma/adapter-pg`. خصائص الأمان:
>
> - وضع `DRY-RUN` افتراضي — لا حذف بدون `--confirm=DELETE-ALL-INVOICES` صريحاً.
> - يطبع عدّادات الصفوف قبل + بعد لكل جدول متأثّر.
> - كل العمليات داخل `prisma.$transaction(...)` واحد — أي فشل = rollback تلقائي، لا تغيير جزئي.
> - يعطّل trigger الـ`DebtLedgerEntry` الـappend-only مؤقتاً (`ALTER TABLE ... DISABLE TRIGGER USER`) داخل نفس الـtransaction ثم يعيد تفعيله.
> - يحذف بالترتيب الصحيح للـFK Restrict (DebtTransferOrder قبل Order) و SetNull (كل البقية).
>
> **الجداول المُصفَّرة (قبل → بعد):**
>
> | الجدول                      | قبل   | بعد   |
> | --------------------------- | ----- | ----- |
> | Order                       | 752   | 0     |
> | OrderLineItem               | 69    | 0     |
> | TransactionHistory          | 301   | 0     |
> | DebtLedgerEntry             | 229   | 0     |
> | GeneralLedgerEntry          | 89    | 0     |
> | ManagerCashCustody          | 7     | 0     |
> | Deposit / DebtHold          | 3/17  | 0/0   |
> | Shift                       | 19    | 0     |
> | PosPaymentBundle            | 2     | 0     |
> | CustomerSubscription        | 20    | 0     |
> | **SerialCounter[ORDER_SERIAL]** | 197 | **0** |
> | **CustomerWallet** (rows)   | 6     | **6 — balance/debt = 0** |
>
> **ما لم يُمَس:** المستخدمون، العملاء (بياناتهم كاملة، أرصدتهم = 0)، الفروع، قائمة الأسعار، الأصناف، الأدوار، الصلاحيات، الرواتب، السلف، مصاريف الفرع، إعدادات النظام.
>
> **قواعد استخدام السكريبت مستقبلاً:**
>
> - **لا يُستدعَى على الإنتاج إلا بأمر مباشر من OWNER أو GENERAL_MANAGER.**
> - **إلزامي** أخذ Railway snapshot (Settings → Backups → Create Manual Backup) قبل التنفيذ.
> - يُشغَّل من جذر المشروع: `npx ts-node scripts/reset-invoices.ts` (dry-run) ثم `--confirm=DELETE-ALL-INVOICES` للتنفيذ.
> - يحترم `DATABASE_URL` من `.env` في `process.cwd()` — غيّر الـ`.env` إلى DB محلي قبل التجريب.
> - لا يمس جداول HR/Config/Catalog — نطاقه محدود للدورة التجارية فقط (فواتير + كاش + مديونية + سلاسل الاشتراك).
>
> نسخة احتياطية من السكريبت محفوظة خارج الريبو على `C:\Users\safar\OneDrive\سطح المكتب\reset-invoices.ts` للرجوع السريع دون الحاجة لـclone.
>
> #### 🔤 V19.23 — إدارة التسلسل: إضافة مدراء الفروع — 2026-04-23
>
> **الطلب:** صفحة "إدارة التسلسل" (`/owner/serials`) كانت تعرض السائقين فقط لتعيين الحرف المميّز (`driverPrefix`). المالك طلب إضافة **مدراء الفروع** لأن المدير أحياناً يُصدر فواتير من POS الفرع، فلا بد من حرف مميّز له حتى يبقى `<Prefix>-<N>` فريداً عالمياً.
>
> **التغييرات (Backend — `src/serials/`):**
>
> - `SerialsService.listDrivers()` وسّع `where` من `DRIVER` فقط إلى `{ in: [DRIVER, MANAGER] }`، وأضاف `safariRole` في الـselect + `orderBy: [safariRole, isActive desc, fullName]` ليظهر السائقون أولاً ثم المدراء.
> - `SerialsService.setDriverPrefix()` بدّل شرط `!== DRIVER` إلى `!PREFIX_ROLES.includes(role)` بحيث يسمح لكل من DRIVER و MANAGER فقط.
> - `DriverPrefixRowDto` أضاف حقل `safariRole: 'DRIVER' | 'MANAGER'`.
> - **لا migration** — `User.driverPrefix` موجود كحقل فريد (`@unique`)، والاسم محفوظ للتوافق الخلفي. المعنى الدلالي صار "Operator prefix".
>
> **التغييرات (Frontend — `web/src/pages/owner-serials-page.tsx` + i18n):**
>
> - `DriverPrefixRow` نوعه وُسّع بحقل `safariRole`.
> - الجدول أضاف **عمود "الدور"** بشارة ملوّنة: 🚚 أزرق (سائق) / 🏢 بنفسجي (مدير فرع).
> - العناوين والرسائل الفارغة حُدِّثت: "السائقون ومدراء الفروع"، "الموظف" بدل "السائق"، "لا يوجد موظفون لعرضهم".
> - مفاتيح جديدة في `ar.ts` + `en.ts`: `roleCol`، `roleDriver`، `roleManager`.
>
> **القواعد التشغيلية:**
>
> - الحرف المميّز (`@unique` على `User.driverPrefix`) يبقى فريداً عبر جميع الأدوار — إذا حاول المالك إعطاء مدير حرفاً مستخدماً بواسطة سائق فالـ DB يرفض (`P2002` → `ConflictException`).
> - فقط **OWNER + GENERAL_MANAGER** يصلان للصفحة (`@Roles(OWNER, GENERAL_MANAGER)` على الـcontroller + `Navigate` في الـguard).
> - تعيين/إزالة الحرف المميّز لمدير فرع يظهر في `AuditLog` تلقائياً عبر الـinterceptor العام.
>
> #### 🔢 V19.24 — رقم الفاتورة (`serialNumber`) تسلسل لكل مُشغّل — 2026-04-23
>
> **الطلب:** رقم المعرف المطبوع (`A-1`, `B-2`, …) يكون **تسلسلاً خاصاً بذلك المستخدم** (السائق/مدير الفرع الذي يُصدِر)، لا عداداً عالمياً واحداً يخدم الجميع.
>
> **قبل V19.24:** `SerialCounter` صف واحد `ORDER_SERIAL` — كل فاتورة تأخذ `prefix + "-" + N` حيث `N` يزيد عالمياً، فظهرت تسلسلات مثل `A-1` ثم `B-2` ثم `A-3` (الرقم لا يعكس ترتيب إصدارات **نفس** المشغّل).
>
> **بعد V19.24 (`src/serials/serial-counter.service.ts`):**
>
> - مفتاح عدّاد لكل مُشغّل: `OU_<userId>` في جدول `SerialCounter` (نفس آلية `increment` الذرّية).
> - **مزامنة أرضية** قبل أول `increment` بعد الترحيل: `max(N)` من طلبات ذلك المستخدم ذات `prefix-N` يُرفع به العدّاد حتى لا يُعاد رقم قديم كان مُخزّناً تحت النظام العالمي.
> - صف `ORDER_SERIAL` القديم يُترك في DB للتوافق؛ **الطوابع الجديدة** لا تستخدمه.
>
> **فحص الفجوات (`src/serials/serial-gap.service.ts`):** أعيدت كتابة `scanGaps()` لتفحص، **لكل** مستخدم له `driverPrefix`، النطاق `1…C` حيث `C = max(قيمة OU_، أعلى N في فواتيره)` وتجمع الفجوات؛ `firstGaps` صار `string[]` (مثل `A-2` بدلاً من رقم عارٍ). حقن `SerialCounterService` أزيل من المُراقب لأنه لم يعد مطلوباً.
>
> **واجهة المالك:** شارة السجل في `/owner/serials` تعرض **عدد الفواتير ذات `serialNumber`** (وليس «آخر N عالمي»). بطاقة «مراقبة تسلسل الفواتير» حدّثت تسميات الأعمدة لتعكس المنطق الجديد.
>
> **`scripts/reset-invoices.ts`:** بعد حذف الطلبات، `deleteMany` على `SerialCounter` حيث `key` يبدأ بـ`OU_` ثم إعادة تعيين `ORDER_SERIAL` إلى 0.
>
> **لا migration جديدة** — `SerialCounter.key` هو `String @id`؛ مفاتيح `OU_*` تُنشأ عند أول طابع.
>
> #### 📱 V19.22 — مطابقة تصميم الموبايل بين الكول سنتر والمسؤل — 2026-04-23
>
> **المشكلة:** على الموبايل، درج مسؤل الكول سنتر كان يعرض **4 مجموعات** (الرئيسية / الفواتير / **المالية** / العمليات) بينما الكول سنتر يعرض **3 فقط** (الرئيسية / الفواتير / العمليات). السبب: مجموعة "المالية" كانت تحتوي على عنصر واحد فقط هو `ccPerformance` (تقرير أداء الموظفين)، وهذا يكسر التماثل البصري بين الدورين على الشاشات الصغيرة.
>
> **الحل (`web/src/modules/call-center-supervisor/nav-config.ts`):** حُذفت مجموعة "المالية" كمجموعة منفصلة ودُمج `ccPerformanceItem` داخل مجموعة **الفواتير** (لأنه تقرير أداء مرتبط عضويًا بنشاط تعديل/إلغاء الفواتير). النتيجة:
>
> - مسؤل الكول سنتر الآن يرى نفس هيكل الكول سنتر تمامًا (الرئيسية / الفواتير / العمليات).
> - صلاحياته الإضافية (تعديل + إلغاء الفاتورة في نفس اليوم مع سجل تدقيق) تبقى تظهر داخل صفحات الفواتير كـ row actions، محكومة بـ `access-matrix.ts` — لا تحتاج عنصر سايدبار مستقل.
> - `ccPerformanceItem` محفوظ — ينتقل تحت مجموعة الفواتير في كل من الديسكتوب والموبايل.
>
> **قاعدة:** أي اختلاف بين سايدبار `CALL_CENTER` وسايدبار `CALL_CENTER_SUPERVISOR` يجب أن يُبرَّر داخل ملف `nav-config.ts` المقابل. القاعدة الافتراضية: المسؤل يرى نفس الهيكل + صلاحيات أقوى داخل الصفحات، لا صفحات جديدة.
>
