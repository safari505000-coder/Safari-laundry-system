...
/**
 * Stage-C §3 — the "weekly executive report" auto-archiver.
 *
 * Every Sunday at 07:00 Asia/Kuwait we generate a one-page summary
 * PDF for the preceding ISO week and store it under
 * `uploads/executive-reports/<YYYY-W##>.pdf`. Owner / GM can also
 * trigger an on-demand regeneration of "this week so far" from the
 * insights controller, so the archive is never blocked on the cron.
 */
@Injectable()
export class WeeklyExecutiveReportService {
  private readonly logger = new Logger(WeeklyExecutiveReportService.name);
  private readonly archiveDir = join(
    process.cwd(),
    'uploads',
    'executive-reports',
  );
  ...
  constructor(
    ...
    void fs.mkdir(this.archiveDir, { recursive: true }).catch(() => undefined);
  }

  /** Every Sunday 07:00 Asia/Kuwait. */
  @Cron('0 7 * * 0', { timeZone: 'Asia/Kuwait' })
  async runWeekly() {
    try {
      const entry = await this.generateLatest();
      this.logger.log(
        `Weekly executive report archived: ${entry.key} (${entry.sizeBytes} bytes)`,
      );
    } catch (e) {
  ...
  /**
   * Compile the report for the current ISO week (anchored to today's
   * Kuwait calendar day) and overwrite the archive entry.
   */
  ...
  async listArchive() {
    ...
    const out = await Promise.all(
      pdfs.map(async (name) => {
        const stat = await fs.stat(join(this.archiveDir, name));
        return {
  ...
  async openReport(
    ...
    if (!/^\d{4}-W\d{2}$/.test(key)) {
      throw new NotFoundException('Invalid weekly report key');
    }
    const filename = `${key}.pdf`;
    const filepath = join(this.archiveDir, filename);
    if (!existsSync(filepath)) {
      // Auto-regenerate the current week on-demand to avoid 404 loops
      // when nobody has triggered the cron yet.
      const now = currentIsoWeek(new Date());
      if (now.key === key) {
        await this.generateLatest();
      } else {
        throw new NotFoundException(
          `Weekly executive report ${key} not found`,
        );
      }
    }
  ...
  private async collectPayload(from: Date, to: Date) {
    ...
    const payrollTotal = sum(
      payroll.map(
        (p) =>
          Number(p.basicSalary) + Number(p.allowances) - Number(p.deductions),
      ),
    );
    ...
    const activeDrivers = new Set(
      orders.map((o) => o.driverId).filter((x): x is string => x != null),
    ).size;

    // Pull the driver leaderboard for the exact same period so the
    // PDF shows who drove the week's numbers.
    const days = Math.max(
      1,
  ...
  private async writePdf(
    ...
    doc
      ...
      .fontSize(10)
      .text(
        `Week ${data.key}    ·    ${fromLabel} → ${toLabel}`,
        { align: 'left' },
      );
    doc.moveDown(0.8);
    ...
    if (data.paymentMix.length === 0) {
      ...
      data.paymentMix.forEach((p, i) => {
        ...
        doc.fillColor(palette[i % palette.length]).text(
          `■ ${p.method}  ${p.amount.toFixed(3)} KD  (${pct}%)`,
          48,
          doc.y,
        );
      });
    ...
    doc
      ...
      .fontSize(8)
      .text(
        `Generated ${new Date().toISOString()} · Auto-archived under uploads/executive-reports/${data.key}.pdf`,
        48,
        undefined,
...
/**
 * Return the ISO week that *contains* the given date (Asia/Kuwait),
 * expressed as `YYYY-W##` plus the UTC boundaries of the week (Monday
 * 00:00 Kuwait → next Monday 00:00 Kuwait, exclusive).
 */
function currentIsoWeek(d: Date): { key: string; from: Date; to: Date } {
  ...
  thu.setUTCDate(thu.getUTCDate() + 3); // Monday -> Thursday
  const year = thu.getUTCFullYear();
  const firstThu = new Date(Date.UTC(year, 0, 4));
  const dayDiff =
    (thu.getTime() - firstThu.getTime()) / (24 * 60 * 60 * 1000);
  const firstThuDow = (firstThu.getUTCDay() + 6) % 7;
  const weekNum =
    Math.floor((dayDiff + firstThuDow) / 7) + 1;
