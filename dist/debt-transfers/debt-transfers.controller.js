    ...
    async findOne(id, user) {
        ...
        if (user.role === client_1.SafariRole.DRIVER) {
            if (transfer.sourceDriver?.id !== user.userId &&
                ...
                throw new (await Promise.resolve().then(() => __importStar(require('@nestjs/common')))).NotFoundException('Debt transfer not found.');
            }
        }
        return transfer;
    }
...
__decorate([
    ...
    (0, swagger_1.ApiOperation)({ summary: 'Create a new debt transfer (ACCOUNTANT only).' }),
