...
async function main() {
    ...
    const dashboardBranchPlusDriver = (grossAgg[client_1.DebtEntityCategory.BRANCH] ?? 0) +
        (grossAgg[client_1.DebtEntityCategory.DRIVER] ?? 0);
    ...
    for (const [cid, arr] of byCust) {
        ...
        for (const o of arr) {
            ...
            pool -= share;
            if (o.issuerRole === client_1.SafariRole.DRIVER)
                netDriver += share;
            else if (o.issuerRole === client_1.SafariRole.MANAGER ||
    ...
    console.log(`  BRANCH : ${(grossAgg[client_1.DebtEntityCategory.BRANCH] ?? 0).toFixed(3)} KD`);
    console.log(`  DRIVER : ${(grossAgg[client_1.DebtEntityCategory.DRIVER] ?? 0).toFixed(3)} KD`);
    console.log(`  sum    : ${dashboardBranchPlusDriver.toFixed(3)} KD  ← dashboard pie today\n`);
    ...
    console.log(`  sum    : ${(netBranch + netDriver + netOther).toFixed(3)} KD  ← matches /unpaid-invoices openDebtKd\n`);
    console.log(`Gap explained = ${(dashboardBranchPlusDriver - (netBranch + netDriver)).toFixed(3)} KD (payments never subtracted in the dashboard).`);
...
//# sourceMappingURL=compare-dashboard-debt.js.map