...
let VerifyService = class VerifyService {
    ...
    async verifyPayrollRoster(token) {
        const [ym, branchScope] = token.split('_');
        if (!ym || !/^\d{4}-\d{2}$/.test(ym)) {
            throw new common_1.NotFoundException('Invalid roster token');
        }
        const [ys, ms] = ym.split('-');
        const y = Number.parseInt(ys ?? '0', 10);
        const m = Number.parseInt(ms ?? '1', 10);
        if (!y || !m || m < 1 || m > 12) {
            throw new common_1.NotFoundException('Invalid roster month');
        }
        const from = new Date(Date.UTC(y, m - 1, 1, 0, 0, 0, 0));
        const to = new Date(Date.UTC(y, m, 0, 23, 59, 59, 999));
        const where = { paymentDate: { gte: from, lte: to } };
        if (branchScope)
        ...
        let basic = new client_1.Prisma.Decimal(0);
        let allow = new client_1.Prisma.Decimal(0);
        let commission = new client_1.Prisma.Decimal(0);
        let release = new client_1.Prisma.Decimal(0);
        let hold = new client_1.Prisma.Decimal(0);
        let loan = new client_1.Prisma.Decimal(0);
        let deductions = new client_1.Prisma.Decimal(0);
        let paidCount = 0;
        for (const r of rows) {
