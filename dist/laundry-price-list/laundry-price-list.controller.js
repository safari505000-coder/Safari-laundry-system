...
let LaundryPriceListController = class LaundryPriceListController {
    ...
    findAll(branchId, user) {
        ...
        if (q &&
            user.role !== client_1.SafariRole.OWNER &&
            user.role !== client_1.SafariRole.GENERAL_MANAGER) {
            throw new common_1.BadRequestException('branchId query parameter is only allowed for OWNER or GENERAL_MANAGER.');
        }
        const effective = q && q.length > 0 ? q : (user.branchId ?? null);
        if ((user.role === client_1.SafariRole.DRIVER || user.role === client_1.SafariRole.MANAGER) &&
            !effective) {
            throw new common_1.BadRequestException('DRIVER and MANAGER accounts must be assigned to a branch to load the merged price list.');
        }
...
__decorate([
    (0, common_1.Get)('catalog-version'),
    (0, roles_decorator_1.Roles)(client_1.SafariRole.OWNER, client_1.SafariRole.GENERAL_MANAGER, client_1.SafariRole.MANAGER, client_1.SafariRole.DRIVER, client_1.SafariRole.WORKER, client_1.SafariRole.CALL_CENTER, client_1.SafariRole.CALL_CENTER_SUPERVISOR, client_1.SafariRole.ACCOUNTANT, client_1.SafariRole.SUPERVISOR, client_1.SafariRole.VIEWER),
    (0, swagger_1.ApiOperation)({
        summary: `Monotonic catalog version — POS / Driver cache invalidation (${branding_1.APP_BRAND})`,
        description: 'Lightweight: same `version` string as `SafariStreamSnapshot.priceListVersion`. ' +
...
], LaundryPriceListController.prototype, "catalogVersion", null);
__decorate([
    (0, common_1.Get)(),
    (0, roles_decorator_1.Roles)(client_1.SafariRole.OWNER, client_1.SafariRole.GENERAL_MANAGER, client_1.SafariRole.MANAGER, client_1.SafariRole.DRIVER, client_1.SafariRole.WORKER, client_1.SafariRole.CALL_CENTER, client_1.SafariRole.CALL_CENTER_SUPERVISOR, client_1.SafariRole.ACCOUNTANT, client_1.SafariRole.SUPERVISOR, client_1.SafariRole.VIEWER),
    (0, swagger_1.ApiOperation)({
        summary: `Laundry garment price list (${branding_1.APP_BRAND})`,
        description: 'Official KD prices per item and tier, merged with optional `LaundryBranchItemPrice` overrides. ' +
...
__decorate([
    ...
    (0, swagger_1.ApiOperation)({
        ...
        description: 'Partial update of the master tariff row (prices, name, sort order, category, isActive). Writes bump the catalog version (`GET /catalog-version` and SafariStream) so Driver / Manager POS pollers reload the catalog. Historical orders are never rewritten — OrderLineItem snapshots unit price and label at creation time.',
    }),
